...
const UpdateAdmin = () =>{
    ...
    useEffect(() =>{
        const fetchAllSentra = async ()=>{
            try {
                const res = await axios.get("https://sihpn-server-final-production.up.railway.app/admin/update", {withCredentials: true})
                setAdmin(res.data[0]);
            } catch (err) {
                console.log(err)
            }
    ...
    const handleClick = async (e) =>{
        ...
        try {
            if(sentra.oldUsername !== admin.username){
                ...
                await axios.post("https://sihpn-server-final-production.up.railway.app/admin/update", sentra, {withCredentials: true})
